    ...
    columnToggle(target) {
        ...
        if (target.classList.contains(COLLAPSED_CLASS)) {
            ...
            for (const row of targetRow) {
                ...
                const showAddButton = targetCol.querySelector('.e-show-add-button');
                if (showAddButton) {
                    removeClass([showAddButton], COLLAPSED_CLASS);
                }
            ...
            for (const row of targetRow) {
                ...
                const showAddButton = targetCol.querySelector('.e-show-add-button');
                if (showAddButton) {
                    addClass([showAddButton], COLLAPSED_CLASS);
                }
                let text;
                if (!this.parent.enableVirtualization) {
        ...
        this.parent.trigger(actionBegin, args, (addArgs) => {
            if (!addArgs.cancel) {
                ...
                if (this.parent.enableHtmlSanitizer) {
                    const field = this.parent.cardSettings.contentField;
                    if (this.parent.enableHtmlSanitizer && typeof field === 'string') {
                        cardData[field] =
                            SanitizeHtmlHelper.sanitize(cardData[field]);
                    }
                }
...
class DragAndDrop {
    ...
    autoScroll() {
        ...
        const x = this.dragObj.pageX - window.scrollX;
        const scrollStep = 30;
        // Near left edge?
...
class ColumnDragAndDrop {
    ...
    wireColumnDragEvents(headerCell) {
    ...
    columnDragHelper(e) {
        this.draggedColumn = closest(e.sender.target, '.' + HEADER_CELLS_CLASS);
        if (isNullOrUndefined(this.draggedColumn)) {
            return null;
        }
        this.isStackedHeader = this.draggedColumn.classList.contains(STACKED_HEADER_CELL_CLASS);
        if (this.isStackedHeader) {
            const stackedHeaderIndex = this.getColumnIndex(this.draggedColumn);
            const stackedHeader = this.parent.stackedHeaders[stackedHeaderIndex];
            const keyFields = stackedHeader.keyFields.split(',').map((field) => field.trim());
            this.contentCells = [];
            keyFields.forEach((key) => {
                const cellsForKey = Array.from(this.parent.element.querySelectorAll('.e-content-cells[data-key="' + key + '"]'));
                this.contentCells.push(...cellsForKey);
            });
        }
        else {
            const columnKey = this.draggedColumn.getAttribute('data-key');
    ...
    columnDragStart(e) {
        ...
        this.parent.trigger(columnDragStart, dragStartArgs);
    ...
    columnDrag(e) {
        ...
        const targetHeader = this.findTargetHeader(e);
        if (!targetHeader) {
            this.dropIndicator.style.display = 'none';
            return;
        }
        const draggedColumnKey = this.draggedColumn.getAttribute('data-key');
        const targetColumnKey = targetHeader.getAttribute('data-key');
        const draggedParentHeader = this.getParentStackedHeader(draggedColumnKey);
        const targetParentHeader = this.getParentStackedHeader(targetColumnKey);
        ...
        this.parent.trigger(columnDrag, dragArgs);
        if (dragArgs.cancel) {
            this.resetDragState();
            return;
        }
    ...
    columnDragStop(e) {
        ...
        const targetIndex = this.getColumnIndex(targetHeader);
        const dropIndex = targetIndex;
        const dropArgs = {
            cancel: false,
            event: e,
            element: this.draggedColumn,
        ...
        this.parent.trigger(columnDrop, dropArgs);
    ...
    createColumnSkeleton() {
        ...
        // Clone header
        const headerClone = this.draggedColumn.cloneNode(true);
        headerClone.classList.add('e-kanban-skeleton-header');
        this.skeletonElement.appendChild(headerClone);
        // Get the column key
        const columnKey = this.draggedColumn.getAttribute('data-key');
        const calculatedHeigt = this.parent.element.getBoundingClientRect().height;
        // Create content wrapper
        const contentWrapper = createElement('div', {
            className: 'e-kanban-skeleton-content'
        });
        const availableSpace = calculatedHeigt - 16; // 16px for padding
        if (this.isStackedHeader) {
            const cardHeight = this.contentCells[0].childNodes[1].firstChild
                ? this.contentCells[0].childNodes[1].firstChild.offsetHeight : 0;
            const numCards = cardHeight === 0 ? 0 : Math.floor(calculatedHeigt / (cardHeight));
            for (let i = 0; i < numCards; i++) {
                const placeholderCard = createElement('div', {
            ...
            const columnModel = this.parent.columns.find((col) => col.keyField.toString() === columnKey);
            const isCollapsed = columnModel && columnModel.allowToggle && !columnModel.isExpanded;
            if (isCollapsed) {
                ...
                const firstCard = this.parent.element.querySelector('.e-content-cells[data-key="' + columnKey + '"] .e-card');
                let cardHeight;
                if (firstCard && firstCard.offsetHeight) {
                    cardHeight = Math.min(150, firstCard.offsetHeight); // Max 150px
                }
                const numCards = firstCard === null ? 0 : Math.floor(calculatedHeigt / (cardHeight));
                for (let i = 0; i < numCards; i++) {
                    const placeholderCard = createElement('div', {
                        className: 'e-kanban-skeleton-card',
                        styles: 'height: ' + cardHeight + 'px;'
    ...
    showColumnSkeleton(e) {
        if (!this.skeletonElement) {
            return;
        }
        if (this.draggedColumn) {
            const headerElement = closest(this.draggedColumn, '.e-kanban-header');
    ...
    findTargetHeader(e) {
        ...
        if (this.isStackedHeader) {
            const stackedHeader = closest(target, '.' + STACKED_HEADER_CELL_CLASS);
            if (stackedHeader && stackedHeader !== this.draggedColumn) {
                return stackedHeader;
            }
        }
        else {
            const headerCell = closest(target, '.' + HEADER_CELLS_CLASS);
    ...
    getParentStackedHeader(columnKey) {
        if (!this.parent.stackedHeaders || this.parent.stackedHeaders.length === 0) {
            return null;
        }
        return this.parent.stackedHeaders.find((header) => {
            const keyFields = header.keyFields.split(',').map((key) => key.trim());
            return keyFields.indexOf(columnKey) !== -1;
        });
    }
    getColumnIndex(headerCell) {
    ...
    reorderColumns(fromIndex, toIndex) {
        const visibleColumns = this.getVisibleColumns(this.parent.columns);
        const sourceIndex = visibleColumns[fromIndex].index;
        const targetIndex = visibleColumns[toIndex].index;
        const columns = this.parent.columns.slice();
    ...
    reorderStackedHeaders(sourceIndex, targetIndex) {
        const stackedHeadersList = this.parent.stackedHeaders.slice();
        const movingStackedHeader = stackedHeadersList.splice(sourceIndex, 1)[0];
        stackedHeadersList.splice(targetIndex, 0, movingStackedHeader);
        this.parent.stackedHeaders = stackedHeadersList;
        const headerToColumnsMap = new Map();
        stackedHeadersList.forEach((header) => {
            const keyFields = header.keyFields.split(',').map((key) => key.trim());
            headerToColumnsMap.set(header.keyFields, []);
            keyFields.forEach((keyField) => {
                const columnIndex = this.parent.columns.findIndex((column) => column.keyField.toString() === keyField);
                if (columnIndex !== -1) {
                    headerToColumnsMap.get(header.keyFields).push({
                        column: this.parent.columns[columnIndex],
                        index: columnIndex
                    });
                }
            });
        ...
        stackedHeadersList.forEach((header) => {
            const columnsForHeader = headerToColumnsMap.get(header.keyFields) || [];
            columnsForHeader.sort((a, b) => {
                return a.index - b.index;
            });
            columnsForHeader.forEach((item) => {
                newColumnsArray.push(item.column);
            });
        });
        this.parent.columns.forEach((column) => {
            const isInStackedHeader = stackedHeadersList.some((header) => {
                const keyFields = header.keyFields.split(',').map((key) => key.trim());
    ...
    unwireColumnDragEvents(headerCell) {
        if (!isNullOrUndefined(headerCell)) {
            const ej2Instances = headerCell.ej2_instances;
            if (!isNullOrUndefined(ej2Instances) && ej2Instances.length > 0) {
                const dragInstance = ej2Instances[0];
                if (dragInstance && !dragInstance.isDestroyed) {
                    dragInstance.destroy();
                }
            }
        ...
        for (const column of this.parent.columns) {
            if (this.isColumnVisible(column)) {
                ...
                if (column.showAddButton) {
                    ...
                    if (!column.isExpanded) {
                        addClass([button], COLLAPSED_CLASS);
                    }
            ...
            if (this.isColumnVisible(column)) {
                ...
                if (this.parent.enableVirtualization) {
                    ...
                    if (this.parent.height === 'auto') {
                        ...
                        const parentHeight = this.parent.element.parentElement.clientHeight;
                        const percentageValue = parseFloat(this.parent.height) / 100;
                        const pixelHeight = parentHeight * percentageValue;
...
let Kanban = class Kanban extends Component {
    ...
    wireColumnDragEvent() {
        if (this.allowColumnDragAndDrop) {
            const headerContainer = this.element.querySelector('.' + HEADER_CLASS);
            if (headerContainer) {
                if (this.stackedHeaders && this.stackedHeaders.length > 0) {
                    // Wire stacked headers for dragging stacked header groups
                    const stackedHeaderSelector = '.' + STACKED_HEADER_CELL_CLASS;
                    const stackedHeaderCells = [].slice.call(this.element.querySelectorAll(stackedHeaderSelector));
                    addClass(stackedHeaderCells, DRAG_CLASS);
                    // Wire events to stacked headers
                    stackedHeaderCells.forEach((headerCell) => {
                    ...
                    const columnHeaderSelector = '.' + HEADER_CELLS_CLASS + ':not(.' + STACKED_HEADER_CELL_CLASS + ')';
                    const columnHeaderCells = [].slice.call(this.element.querySelectorAll(columnHeaderSelector));
                    addClass(columnHeaderCells, DRAG_CLASS);
                    ...
                    const headerSelector = '.' + HEADER_CELLS_CLASS;
                    const headerCells = [].slice.call(this.element.querySelectorAll(headerSelector));
                    addClass(headerCells, DRAG_CLASS);
    ...
    unwireColumnDragEvent() {
        if (this.stackedHeaders && this.stackedHeaders.length > 0) {
            ...
            const stackedHeaderSelector = '.' + STACKED_HEADER_CELL_CLASS;
            const stackedHeaderCells = [].slice.call(this.element.querySelectorAll(stackedHeaderSelector));
            removeClass(stackedHeaderCells, DRAG_CLASS);
            ...
            const columnHeaderSelector = '.' + HEADER_CELLS_CLASS + ':not(.' + STACKED_HEADER_CELL_CLASS + ')';
            const columnHeaderCells = [].slice.call(this.element.querySelectorAll(columnHeaderSelector));
            removeClass(columnHeaderCells, DRAG_CLASS);
            ...
            const headerSelector = '.' + HEADER_CELLS_CLASS;
            const headerCells = [].slice.call(this.element.querySelectorAll(headerSelector));
            removeClass(headerCells, DRAG_CLASS);
...
__decorate$6([
    Property(false)
], Kanban.prototype, "allowColumnDragAndDrop", void 0);
__decorate$6([
    Property(false)
...
__decorate$6([
    Event()
], Kanban.prototype, "columnDragStart", void 0);
__decorate$6([
    Event()
], Kanban.prototype, "columnDrag", void 0);
__decorate$6([
    Event()
], Kanban.prototype, "columnDrop", void 0);
__decorate$6([
    Event()
...
export { Kanban, actionBegin, actionComplete, actionFailure, bottomSpace, cardClick, cardDoubleClick, cardRendered, cardSpace, columnDrag, columnDragStart, columnDrop, contentReady, dataBinding, dataBound, dataReady, dataSourceChanged, dataStateChange, dialogClose, dialogOpen, documentClick, drag, dragStart, dragStop, queryCellInfo, toggleWidth };
