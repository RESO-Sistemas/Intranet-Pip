...
/** @hidden */
const ICON_FOLDER = 'e-icons e-folder';
/** @hidden */
const ICON_FILE = 'e-icons e-file-format';
/** @hidden */
 ...
 * @private
 */
function getPath(element, text, hasId, instance) {
    const matched = getParents(element, text, false, hasId, instance);
    let path = '/';
 ...
 * @private
 */
function getPathId(node, instance) {
    const matched = getParents(node, node.getAttribute('data-uid'), true, false, instance);
    const ids = [];
 ...
 * @private
 */
function getPathNames(element, text, instance) {
    const matched = getParents(element, text, false, false, instance);
    const names = [];
    for (let i = matched.length - 1; i >= 0; i--) {
 ...
 * @private
 */
function getParents(element, text, isId, hasId, instance) {
    const matched = [text];
    let el = element.parentNode;
    while (!isNullOrUndefined(el)) {
        if (matches(el, '.' + LIST_ITEM)) {
            let data = [{}];
            if (!isNullOrUndefined(instance) && !isNullOrUndefined(instance.navigationpaneModule) && !isNullOrUndefined(instance.navigationpaneModule.treeObj)) {
                data = instance.navigationpaneModule.treeObj.getTreeData(el);
            }
            const parentText = isId ? el.getAttribute('data-uid') : (hasId ? el.getAttribute('data-id') : getValue('name', data[0]));
...
function createDeniedDialog(parent, data, action) {
    ...
    let message = getValue('message', permission);
    const restriction = getValue('uploadContentFilter', permission) === 1 ? 'Folder Upload' : 'File Upload';
    if (message === '') {
        if (!isNullOrUndefined(permission) && getValue('upload', permission) && getValue('uploadContentFilter', permission) === 1) {
            message = getLocaleText(parent, 'FolderUpload-Access-Message').replace('{0}', getValue('name', data)).replace('{1}', restriction);
        }
        else if (!isNullOrUndefined(permission) && getValue('upload', permission) && getValue('uploadContentFilter', permission) === 2) {
...
class ContextMenu {
    ...
    onBeforeItemRender(args) {
        ...
        if (args.item.id === this.getMenuId('fileupload')) {
            this.renderFileFolderIcons(args, ICON_FILE, !this.parent.uploadObj.directoryUpload);
        }
        if (args.item.id === this.getMenuId('folderupload')) {
            this.renderFileFolderIcons(args, ICON_FOLDER, this.parent.uploadObj.directoryUpload);
        }
    }
    renderFileFolderIcons(menuArgs, fileFolderIcon, isDirectory) {
        const iconSpan = createElement('span');
        const element = menuArgs.element;
        element.insertBefore(iconSpan, isDirectory ? element.childNodes[1] : element.childNodes[0]);
        iconSpan.setAttribute('class', fileFolderIcon + ' ' + MENU_ICON);
    }
    onBeforeClose(args) {
        const eventArgs = {
            cancel: false,
...
let FileManager = FileManager_1 = class FileManager extends Component {
    ...
    allowOrDenyUpload(args, uploadDetails) {
        const permission = getValue('permission', uploadDetails);
        if (isNullOrUndefined(args) || isNullOrUndefined(permission) || isNullOrUndefined(getValue('uploadContentFilter', permission))) {
            return false;
        }
        const hasFolder = args.filesData.some((f) => {
            const name = !isNullOrUndefined(f.name) ? f.name : '';
            return name.includes('/') || name.includes('\\');
        });
        const hasFile = args.filesData.some((f) => {
            const name = !isNullOrUndefined(f.name) ? f.name : '';
            return !(name.includes('/') || name.includes('\\'));
        });
...
class NavigationPane {
    ...
    onInit() {
        ...
        this.treeObj = new TreeView({
            fields: Object.assign({ dataSource: [rootData], id: '_fm_id', parentID: '_fm_pId', expanded: '_fm_expanded', selected: '_fm_selected', text: 'name', hasChildren: 'hasChild' }, (isNullOrUndefined(this.parent.navigationPaneTemplate) && { iconCss: '_fm_icon' }), { htmlAttributes: '_fm_htmlAttr', tooltip: 'name' }),
            enableHtmlSanitizer: this.parent.enableHtmlSanitizer,
        ...
        if (!this.parent.isMobile && this.treeObj) {
            if (this.parent.allowDragAndDrop && isNullOrUndefined(this.dragObj)) {
                this.dragObj = new Draggable(this.treeObj.element, {
                    cursorAt: this.parent.dragCursorPosition,
                    dragTarget: '.' + FULLROW + ',.' + TEXT_CONTENT,
                    distance: 5,
                    dragArea: this.parent.element,
                    drag: draggingHandler.bind(this, this.parent),
                    dragStart: (args) => {
    ...
    dragHelper(args) {
        const dragTarget = args.sender.target;
        if (!dragTarget.classList.contains(FULLROW) && !dragTarget.classList.contains(TEXT_CONTENT)) {
            return null;
        }
    ...
    onNodeExpand(args) {
        ...
        if (args.node.querySelector('.' + LIST_ITEM) === null) {
            ...
            const newText = isNullOrUndefined(id) ? nodeData[0].name : id;
            const path = getPath(args.node, newText, isId, this.parent);
...
class DetailsView {
    ...
    getColumns() {
        ...
        const iWidth = ((this.parent.isMobile || this.parent.isBigger) ? '70' : '56');
        const icon = {
    ...
    onDataBound() {
        ...
        this.isLoaded = false;
        if (this.parent.sortOrder === 'None' && !isNullOrUndefined(this.gridObj.element)) {
            const sortIconElement = this.gridObj.element.querySelector('.e-ascending, .e-descending');
    ...
    onDetailsResize() {
        if (this.parent.view === 'Details' && !this.parent.isMobile && !isNullOrUndefined(this.gridObj)) {
            ...
            const gridHeaderColGroup = gridHeader.querySelector('colgroup');
