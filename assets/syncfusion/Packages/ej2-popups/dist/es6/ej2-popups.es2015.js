...
function setPopup(element, pos, elementRect) {
    ...
    if (offsetElement && (element.classList.contains('e-filter-popup') || element.classList.contains('e-ccdlg'))) {
        const newpos = calculateRelativeBasedPosition(offsetElement, element);
        const dlgWidth = element.clientWidth;
        let leftPos = newpos.left - dlgWidth + offsetElement.clientWidth;
        if (leftPos < 1) {
        ...
        const parentElementOffsetWidth = offsetElement.offsetWidth;
        if ((parentElementOffsetWidth - leftPos) < dlgWidth) {
            const actualWidthAllocated = parentElementOffsetWidth - leftPos;
            const requiredWidth = dlgWidth - actualWidthAllocated;
            element.style.left = leftPos - requiredWidth + 'px';
        }
    }
    if (element.classList.contains('e-filter-popup') && element.offsetParent) {
        const currentLeftPos = parseFloat(element.style.left);
        const gridWidth = element.offsetParent.clientWidth;
        const dlgWidth = element.clientWidth;
...
function leftFlip(target, edge, tEdge, pos, elementRect, deepCheck) {
    const isWrapper = /\b(e-date-wrapper|e-datetime-wrapper)\b/.test(target.className);
    let overlap = 0;
...
let Tooltip = class Tooltip extends Component {
    ...
    renderCloseIcon() {
        if (!this.isSticky) {
            const existingCloseIcon = this.tooltipEle.querySelector('.' + ICON + '.' + CLOSE);
            if (existingCloseIcon) {
                remove(existingCloseIcon);
            }
            return;
        }
        const tipClose = this.createElement('div', { className: ICON + ' ' + CLOSE, attrs: { role: 'button', 'aria-label': 'Press escape to close the Tooltip' } });
...
const MODAL_COUNT_ATTR = 'data-e-dlg-open-count';
/**
...
let Dialog = class Dialog extends Component {
    ...
    incrementModalCount(element) {
        const targetElement = element || document.body;
        let count = parseInt(targetElement.getAttribute(MODAL_COUNT_ATTR) || '0', 10);
        ...
        this.popupObj = new Popup(this.element, {
            ...
            close: (event) => {
                ...
                if (!isNullOrUndefined(this.storeActiveElement) && !isNullOrUndefined(this.storeActiveElement.focus)
                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
                    && !isNullOrUndefined(this.closeArgs) && !this.closeArgs.preventFocus) {
    ...
    setFooterTemplate() {
        ...
        if (this.element.classList.contains('e-dlg-tooltip')) {
            Array.from(this.ftrTemplateContent.children).forEach((el) => {
                el.classList.add('e-footer-btn');
            });
        }
    }
    attachEllipsisTooltip(element, content, position, width) {
        if (!element || element.getAttribute('data-has-tooltip') === 'true') {
            return;
        }
        const dialogWidth = typeof this.width === 'string' ? parseFloat(this.width) : this.width;
        const tip = new Tooltip({ content, position, opensOn: 'Hover', width: width >= dialogWidth ? dialogWidth : width }, element);
        const tooltipKey = 'tooltip_' + (this.tooltipIndex++);
    ...
    applyOverflowTooltips() {
        ...
        if (this.headerEle) {
            ...
            const headerText = /<[^>]+>/.test(raw) ? (this.headerEle.textContent || '').trim() : this.header;
        ...
        if (this.ftrTemplateContent) {
            const buttons = this.ftrTemplateContent.querySelectorAll('.e-footer-btn');
            buttons.forEach((btn) => {
                const btnEl = btn;
                const textSpan = btnEl.querySelector('.e-btn-text') || btnEl;
                const text = (textSpan.innerText || textSpan.textContent || '').trim();
    ...
    closeOverflowTooltips(cssClass) {
        ...
        if (this.ftrTemplateContent) {
            elements.push(...Array.from(this.ftrTemplateContent.querySelectorAll('.e-footer-btn')));
        }
        elements.forEach((el) => {
            const tooltipKey = el.getAttribute('data-tooltip-key');
            const tip = tooltipKey ? this.tooltipInstance[tooltipKey] : undefined;
            if (tip && typeof tip.getModuleName === 'function' && tip.getModuleName() === 'tooltip') {
                ...
                if (tooltipKey) {
                    delete this.tooltipInstance[tooltipKey];
                    this.tooltipIndex--;
                }
            }
        });
    ...
    destroy() {
        ...
        const classArray = [RTL$1, MODAL_DLG, DLG_RESIZABLE, DLG_RESTRICT_LEFT_VALUE, FULLSCREEN, DEVICE$1];
