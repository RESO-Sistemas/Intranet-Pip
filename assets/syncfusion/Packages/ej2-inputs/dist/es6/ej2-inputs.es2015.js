...
(function (Input) {
        ...
        if (!isNullOrUndefined(args.element)) {
            if (!isNullOrUndefined(args.element.__eventHandlers)) {
                if (!isNullOrUndefined(args.element.__eventHandlers.inputFocusHandler)
                    ...
                    const focusHandler = args.element
                        .__eventHandlers.inputFocusHandler;
                    const blurHandler = args.element
                        .__eventHandlers.inputBlurHandler;
                    const inputHandler = args.element.__eventHandlers.inputHandler;
    ...
    function unWireFloatLabelEvents(args) {
        if (!isNullOrUndefined(args.element) &&
            ...
            const inputFloatHandler = args.element.__eventHandlers.floatInputHandler;
            const blurFloatHandler = args.element.__eventHandlers.floatBlurHandler;
    ...
    function unWireClearBtnEvents(element, button) {
        if (!isNullOrUndefined(element) &&
            ...
            if (!isNullOrUndefined(element.__eventHandlers.clearClickHandler)) {
                const clickHandlerEvent = element.__eventHandlers.clearClickHandler;
                if (isBindClearAction === undefined || isBindClearAction) {
                    if (!isNullOrUndefined(button)) {
            ...
            if (!isNullOrUndefined(element.__eventHandlers.clearInputHandler)
                ...
                const inputHandlerEvent = element.__eventHandlers.clearInputHandler;
                const focusHandlerEvent = element.__eventHandlers.clearFocusHandler;
                const blurHandlerEvent = element.__eventHandlers.clearBlurHandler;
...
let Uploader = class Uploader extends Component {
    ...
    removeFilesData(file, customTemplate) {
        ...
        this.element.value = '';
        const iconElement = selectedElement.querySelector('.e-icons');
        if (iconElement) {
    ...
    clearData(singleUpload) {
        if (!isNullOrUndefined(this.listParent)) {
            const fileItems = this.listParent.querySelectorAll('.e-icons');
        ...
        if (this.isFormUpload() && this.uploadWrapper.querySelector('.' + INPUT_WRAPPER)) {
            let hiddenElement = this.uploadWrapper.querySelector('.' + INPUT_WRAPPER).querySelector('.' + HIDDEN_INPUT);
            if (hiddenElement) {
    ...
    createCustomfileList(fileData) {
        ...
        for (const listItem of fileData) {
            let listElement = this.createElement('li', { className: FILE, attrs: { 'data-file-name': listItem.name } });
