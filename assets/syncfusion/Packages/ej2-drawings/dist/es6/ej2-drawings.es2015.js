...
function wordWrappingBlazor(text, textValue, maxHeight) {
    ...
    let existingText;
    let height = 0;
    let thicknessInPixel = text.thickness * (96 / 72);
    // The padding has been added to remove the overflow text such that text would be contained within the bounds.
    const topPadding = text.thickness !== undefined ? thicknessInPixel === 1 || thicknessInPixel === 0 ? thicknessInPixel * 4 : thicknessInPixel + 4 : 5;
    for (j = 0; j < eachLine.length; j++) {
        if (childNodes.length > 0 && ((childNodes.length * text.fontSize * 1.2) + topPadding) > maxHeight) {
            break;
        }
        words = text.textWrapping !== 'NoWrap' ? eachLine[parseInt(j.toString(), 10)].split(' ') : eachLine;
        const exactMaxHeight = text.thickness !== undefined ? Math.floor(maxHeight) : maxHeight;
        for (i = 0; i < words.length; i++) {
            const exactTextHeight = text.thickness !== undefined ? Math.ceil(height) : height;
            bounds1 = bBoxText(words[parseInt(i.toString(), 10)], text);
            //Used to get the word width including thickness.
            const wordWidth = text.thickness !== undefined ? thicknessInPixel === 1 || thicknessInPixel === 0 ? (thicknessInPixel * 2) + bounds1 : (thicknessInPixel * 2) + 4 + bounds1 : bounds1;
            if (wordWidth > text.width && words[parseInt(i.toString(), 10)].length > 0 && text.textWrapping !== 'NoWrap' && exactTextHeight < exactMaxHeight) {
                ...
                text.content = words[parseInt(i.toString(), 10)];
                const res = wrapTextBlazor(text, txtValue, childNodes, maxHeight, height);
                childNodes = res.childNode;
                height = res.height;
                text.content = previousTextContent;
            }
                ...
                txtValue += (((i !== 0 || words.length === 1) && wrap && txtValue.length > 0) ? ' ' : '') + words[parseInt(i.toString(), 10)];
                let spaceWithText = text.thickness !== undefined && i + 1 != words.length ? ' ' : '';
                newText = txtValue + spaceWithText + (words[i + 1] || '');
                let width = bBoxText(newText, text);
                if (eachLine.length > 1 && i === words.length - 1) {
                    txtValue = txtValue + '\n';
                }
                const isNewlineOnly = txtValue === '\n';
                const isLastWordAndNextLineEmpty = (i + 1 >= words.length && eachLine[parseInt((j + 1).toString(), 10)] === '');
                let calculatedWidth = text.thickness !== undefined ? thicknessInPixel === 1 ? Math.floor(thicknessInPixel) + Math.ceil(width) : Math.floor(thicknessInPixel) * 2 + 4 + Math.ceil(width) + 2 : Math.ceil(width) + 2;
                ...
                const exactTextWidth = text.thickness !== undefined ? Math.floor(text.width) : text.width;
                if ((calculatedWidth + 1) >= exactTextWidth && txtValue.length > 0) {
                    ...
                    const sanitizedForHeight = (isNewlineOnly || isLastWordAndNextLineEmpty) ? txtValue : txtValue.replace(/\n/g, '');
                    ...
                    if (i === words.length - 1) {
                        ...
                        const sanitizedForHeightEnd = (isNewlineOnly || isLastWordAndNextLineEmpty) ? txtValue : txtValue.replace(/\n/g, '');
...
function wordWrapping(text, textValue, maxHeight) {
    ...
    let thicknessInPixel = text.thickness * (96 / 72);
    // The padding has been added to remove the overflow text such that text would be contained within the bounds.
    const topPadding = text.thickness !== undefined ? (thicknessInPixel) * 4 : 5;
    for (j = 0; j < eachLine.length; j++) {
        if (childNodes.length > 0 && ((childNodes.length * text.fontSize * 1.2) + topPadding) > maxHeight) {
            break;
        }
        ...
        for (i = 0; i < words.length; i++) {
            ...
            bounds1 = bBoxText(words[parseInt(i.toString(), 10)], text);
            //Used to get the word width including thickness.
            const wordWidth = text.thickness !== undefined ? thicknessInPixel * 4 + bounds1 : bounds1;
            if (wordWidth > text.width && words[parseInt(i.toString(), 10)].length > 0 && text.textWrapping !== 'NoWrap' && exactTextHeight < exactMaxHeight) {
                if (eachLine.length > 1) {
                    words[parseInt(i.toString(), 10)] = words[parseInt(i.toString(), 10)] + '\n';
                }
                ...
                const isNewlineOnly = txtValue === '\n';
                const isLastWordAndNextLineEmpty = (i + 1 >= words.length && eachLine[parseInt((j + 1).toString(), 10)] === '');
                let calculatedWidth = text.thickness !== undefined ? Math.ceil(thicknessInPixel) * 4 + Math.ceil(width) + 2 : Math.ceil(width) + 2;
                let paddingBetweenWords = text.thickness !== undefined ? (thicknessInPixel) * 2 : 0;
                if (childNodes.length == 0 && text.thickness !== undefined && topPadding !== 0 && topPadding > exactMaxHeight) {
                    height = height + getHeightMultiplier(txtValue) * bBoxTextHeight(txtValue, text) + topPadding;
                    break;
                }
...
function wrapTextBlazor(txt, textValue, childNode, maxHeight, height) {
    ...
    txtValue += content[0];
    let isFreeTextHeightAllowed;
    let additionalPadding = txt.thickness !== undefined && (txt.thickness === 0 || txt.thickness === 1) ? Math.ceil(txt.thickness * (96 / 72)) : Math.ceil(txt.thickness * (96 / 72)) + 4;
    for (k = 0; k < content.length; k++) {
        bounds1 = bBoxText(txtValue, txt);
        if (bounds1 + additionalPadding >= txt.width && txtValue.length > 0) {
            ...
            txtValue = txtValue + (content[k + 1] || '');
            let width = bBoxText(txtValue, txt) + additionalPadding;
            if ((Math.ceil(width) + 2 >= txt.width && txtValue.length > 0)) {
                height = height + bBoxTextHeight(txtValue, txt);
                txtValue = txtValue.slice(0, -1);
                width = bBoxText(txtValue, txt);
...
function wrapText(txt, textValue, childNode, maxHeight) {
    ...
    txtValue += content[0];
    let isFreeTextHeightAllowed;
    let additionalPadding = txt.thickness !== undefined ? Math.ceil(txt.thickness * (96 / 72)) * 4 : 0;
    for (k = 0; k < content.length; k++) {
        bounds1 = bBoxText(txtValue, txt);
        if (bounds1 + additionalPadding >= txt.width && txtValue.length > 0) {
...
function measureText(text, style, content, maxWidth, maxHeight, textValue) {
    ...
    if (text.isEJ2 === true && text.isFreeText === true) { // EJ2 Free Text: use strokeWidth only, double the standard 1.5x per-side padding
        let stroke = Math.ceil(options.strokeWidth || 0);
        let totalPad = stroke * 3 * 1.8;
...
function bBoxTextBlazor(textContent, options) {
    let measureElement = 'measureElement';
    // eslint-disable-next-line
    window[measureElement].style.visibility = 'visible';
    // eslint-disable-next-line
    ...
    text.style.cssText = `font-size: ${options.fontSize}px; font-family: ${options.fontFamily};
    font-weight: ${options.bold ? 'bold' : 'normal'}`;
    const expandedString = expandTabsForSvg(text, textContent, { startX: 0, tabSizeSpaces: (7) });
    text.textContent = expandedString.expandedString;
    let bBox = expandedString.width;
    // eslint-disable-next-line
    window[measureElement].style.visibility = 'hidden';
    return bBox;
}
function expandTabsForSvg(textEl, input, { startX = 0, tabSizeSpaces = (7) } = {}) {
    // Measure the width of a single space in the current font.
    const original = textEl.textContent;
    textEl.textContent = ' ';
    const spaceW = textEl.getBBox().width || 0;
    ...
    for (const ch of input) {
        if (ch === '\t') {
            ...
            const nextOffset = Math.ceil(offset / tabW) * tabW;
            const padPx = nextOffset - offset;
            // convert pixel gap to a number of spaces (round up)
            const padSpaces = Math.max(1, Math.ceil(padPx / spaceW));
...
class CanvasRenderer {
    ...
    setStyleFreetextEJ2(canvas, style) {
        let ctx = CanvasRenderer.getContext(canvas);
        if (style.fill === 'none') {
            style.fill = 'transparent';
        }
    ...
    /**   @private  */
    drawRectangleFreetextEJ2(canvas, options) {
        if (options.childNodes === undefined) {
            this.rectWidth = options.width;
        }
        if (options.visible === true) {
    ...
    /**   @private  */
    drawTextFreetextEJ2(canvas, options, isFreeTextAnnotation, rectHeight) {
        if (options.content && options.visible === true) {
            ...
            if (wrapBounds) {
                let position = this.labelAlign(options, wrapBounds, childNodes, lineHeight);
                let padding = 0;
                let paddingY = 0;
                const heightDiff = rectHeight - (options.height + options.strokeWidth * 2);
                const stroke = Math.ceil(options.strokeWidth || 0);
                ...
                for (i = 0; i < childNodes.length; i++) {
                    ...
                    let offsetX;
                    let offsetY;
                    let isTextDecorationApplied = false;
                    if (options.textAlign === 'justify') {
                        if (child.text === '\n')
                            continue;
                        let baseSpaceWidth = ctx.measureText(' ').width;
                        let targetWidth = this.rectWidth - (Math.ceil(options.strokeWidth) * 3 * 1.5);
                        offsetX = position.x + child.x - wrapBounds.x + padding;
                        offsetY = position.y + child.dy * i + ((options.fontSize) * 0.8) + paddingY;
                        let isLastLine = i === childNodes.length - 1;
                        if (!isLastLine && targetWidth > 0) {
                            let leftEdge = position.x + child.x - wrapBounds.x + padding;
                            ...
                            const wordsTotal = widths.reduce((a, b) => a + b, 0);
                            const gaps = words.length - 1;
                            const naturalWidth = wordsTotal + baseSpaceWidth * gaps;
    ...
    /**   @private  */
    drawTextEJ2(canvas, options, isStampAnnotation) {
        if (options.content && options.visible === true) {
            let ctx = CanvasRenderer.getContext(canvas);
            ctx.save();
            this.setStyle(canvas, options);
                ...
                for (i = 0; i < childNodes.length; i++) {
                    ...
                    if (options.textAlign === 'justify') {
                        if (child.text === '\n')
                            continue;
                        let baseSpaceWidth = ctx.measureText(' ').width;
                        let targetWidth = wrapBounds.width;
                        ...
                        let isLastLine = i === childNodes.length - 1;
                        if (!isLastLine && targetWidth > 0) {
                            let leftEdge = position.x + child.x + options.strokeWidth / 2;
                            const words = child.text.trim().split(/\s+/);
                            if (words.length <= 1) {
                                ctx.fillText(child.text, leftEdge, offsetY);
                                continue;
    ...
    /**@private*/
    drawFreeTextBlazor(canvas, options, maxHeight, isFreeTextAnnotation, zoomFactor) {
        if (options.content && options.visible === true) {
            let ctx = CanvasRenderer.getContext(canvas);
            ctx.save();
            this.setStyle(canvas, options);
            ...
            if (wrapBounds) {
                let position = this.labelAlign(options, wrapBounds, childNodes, lineHeight);
                let paddingAdjustmentX = options.thickness !== undefined && options.thickness === 0 ? Math.floor(options.thickness * (96 / 72)) + 2 : Math.floor(options.thickness * (96 / 72)) * 1 + 2;
                let paddingAdjustmentY = options.thickness !== undefined && options.thickness === 0 ? Math.floor(options.thickness * (96 / 72)) + 5 : Math.floor(options.thickness * (96 / 72)) * 1 + 4;
                let totalTextHeightCanvas = 0;
                for (let j = 0; j < childNodes.length; j++) {
                    let child = childNodes[j];
                    totalTextHeightCanvas += (child.dy ? child.dy : (lineHeight || options.fontSize * 1.2));
                }
                const isApplyClip = isFreeTextAnnotation && maxHeight > 0 && Math.ceil(totalTextHeightCanvas + paddingAdjustmentY + 4) > maxHeight;
                ...
                for (i = 0; i < childNodes.length; i++) {
                    ...
                    if (options.textAlign === 'justify') {
                        ...
                        let isLastLine = i === childNodes.length - 1;
                        if (!isLastLine && targetWidth > 0) {
                            let leftEdge = position.x + child.x + paddingAdjustmentX;
                            const words = child.text.trim().split(/\s+/);
                            if (words.length <= 1) {
                                ctx.fillText(child.text, leftEdge, offsetY);
                                continue;
                            ...
                            const wordsTotal = widths.reduce((a, b) => a + b, 0);
                            const gaps = words.length - 1;
                            const naturalWidth = wordsTotal + baseSpaceWidth * gaps + paddingAdjustmentX * 2;
                            const extra = Math.max(0, targetWidth - naturalWidth);
                            const extraPerGap = extra / gaps;
                            let pen = leftEdge;
                            for (let i = 0; i < words.length; i++) {
                            ...
                            if (options.textDecoration === 'Underline'
                                ...
                                let startX = leftEdge;
                                let startY;
                                let endX = leftEdge + targetWidth - paddingAdjustmentX * 2;
                                let endY;
                                switch (options.textDecoration) {
                                    case 'Underline':
                                        startY = offsetY + 2;
                                    ...
                                    case 'Underline LineThrough':
                                        ...
                                        const midY = (offsetY + position.y + child.dy * i) / 2 + 2 + paddingAdjustmentY / 2;
                        ...
                        const tabSize = 7;
                        const textWithTabs = child.text.replace(/\t/g, ' '.repeat(tabSize));
                        textHeight += child.dy;
    ...
    /**@private*/
    drawText(canvas, options, maxHeight, isFreeTextAnnotation, zoomFactor) {
        if (options.content && options.visible === true) {
            let ctx = CanvasRenderer.getContext(canvas);
            ctx.save();
            this.setStyle(canvas, options);
            ...
            if (wrapBounds) {
                let position = this.labelAlign(options, wrapBounds, childNodes, lineHeight);
                let paddingAdjustment = options.thickness !== undefined ? (options.thickness * (96 / 72)) * 2 : 0;
                let textHeight = 0;
                for (i = 0; i < childNodes.length; i++) {
                    let child = childNodes[parseInt(i.toString(), 10)];
                    let offsetX;
                    ...
                    if (options.textAlign === 'justify') {
                        ...
                        let isLastLine = i === childNodes.length - 1;
                        if (!isLastLine && targetWidth > 0) {
                            let leftEdge = position.x + child.x + paddingAdjustment;
                            const words = child.text.trim().split(/\s+/);
                            if (words.length <= 1) {
                                ctx.fillText(child.text, leftEdge, offsetY);
                                continue;
                            ...
                            const wordsTotal = widths.reduce((a, b) => a + b, 0);
                            const gaps = words.length - 1;
                            const naturalWidth = wordsTotal + baseSpaceWidth * gaps + paddingAdjustment * 2;
                            const extra = Math.max(0, targetWidth - naturalWidth);
                            const extraPerGap = extra / gaps;
                            let pen = leftEdge;
                            for (let i = 0; i < words.length; i++) {
                            ...
                            if (options.textDecoration === 'Underline'
                                ...
                                let startX = leftEdge;
                                let startY;
                                let endX = leftEdge + targetWidth - paddingAdjustment * 2;
                                let endY;
                                switch (options.textDecoration) {
                                    case 'Underline':
                                        startY = offsetY + 2;
                                    ...
                                    case 'Underline LineThrough':
                                        this.drawLine(ctx, startX, offsetY + 2, endX, offsetY + 2, options);
                                        const midY = (offsetY + position.y + child.dy * i) / 2 + 2 + paddingAdjustment / 2;
                                        this.drawLine(ctx, startX, midY, endX, midY, options);
                                        break;
                                }
                                isTextDecorationApplied = true;
    ...
    /** @private  */
    drawTextBlazor(canvas, options, maxHeight, isFreeTextAnnotation, zoomFactor) {
        if (options.content && options.visible === true) {
            let ctx = CanvasRenderer.getContext(canvas);
            ctx.save();
            this.setStyle(canvas, options);
    ...
    drawLine(ctx, startPointX, startPointY, endPointX, endPointY, options) {
        ctx.beginPath();
        ctx.moveTo(startPointX, startPointY);
        ctx.lineTo(endPointX, endPointY);
        ctx.strokeStyle = options.color;
    ...
    /**   @private  */
    labelAlign(text, wrapBounds, childNodes, lineHeight) {
        ...
        let labelX = text.x;
        let labelY = text.y;
        let pointx = text.width * 0.5;
        let pointy = text.height * 0.5;
        if (text.isEJ2 === true) {
            if (text.textAlign === 'left' || text.textAlign === 'justify') {
                pointx = 0;
            }
