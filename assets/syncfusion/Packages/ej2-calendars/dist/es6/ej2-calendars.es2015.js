...
let DatePicker = class DatePicker extends Calendar {
    ...
    updateFloatLabelOverflowWidth() {
        ...
        if (label) {
            const isMaterial = window.getComputedStyle(this.element).getPropertyValue('--dummy-style').trim();
            if (isMaterial === '') {
            ...
            if ((!this.isDateDisabled(this.endValue) && !this.isDateDisabled(this.startValue)
                ...
                for (const ele of tdCells) {
                    ...
                    if (!ele.classList.contains(WEEKNUMBER$2) && isDisabledCell) {
                        const eleDate = this.getIdValue(null, ele);
                        const startDateValue = this.currentView() === 'Month' || currentDate != null ? new Date(+this.startValue) : this.getStartEndDate(new Date(+this.startValue), false);
    ...
    updateInput(isUpdate, date) {
        ...
        if ((typeof date !== 'number') || (this.value && +new Date(+this.value).setMilliseconds(0)) !== +date) {
            ...
            if ((this.globalize.culture === 'ar') && !isNullOrUndefined(date)) {
                const formatOptions = { format: this.cldrTimeFormat(), type: 'time' };
                const value = this.globalize.formatDate(date, formatOptions);
                this.updateInputValue(value);
                if (this.enableMask) {
                    this.notify('setMaskSelection', {
                        module: 'MaskedDateTime'
