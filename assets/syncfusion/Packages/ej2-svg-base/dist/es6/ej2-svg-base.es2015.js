...
let Tooltip = class Tooltip extends Component {
    ...
    render() {
        ...
        if (!this.template) {
            if (this.split) {
                ...
                const markerSide = this.renderTooltipElement(this.areaBounds, this.location, 0);
                this.drawMarker(markerSide.isBottom, markerSide.isRight, this.markerSize, 0);
            }
        }
        else {
            if (this.split) {
                for (let splitIndex = 0; splitIndex < this.data.length; splitIndex++) {
     ...
     */
    renderSplitTooltip() {
        const svgRoot = getElement(this.element.id + '_svg');
        const prevCount = (this.previousContent && this.previousContent.length) ? this.previousContent.length : 0;
        const currentCount = this.content.length;
        if (svgRoot) {
            if (currentCount > prevCount) {
                for (let index = prevCount; index < currentCount; index++) {
                    let tooltipGroup = document.getElementById(this.element.id + '_group_' + index);
                    if (!tooltipGroup) {
                        ...
                        const tooltipPath = this.renderer.drawPath({
                ...
                for (let index = currentCount; index < prevCount; index++) {
        ...
        for (let splitIndex = 0; splitIndex < currentCount; splitIndex++) {
        ...
        for (let splitIndex = 0; splitIndex < currentCount; splitIndex++) {
            ...
            this.trigger('tooltipRender', argsData);
            const markerSide = this.renderTooltipElement(this.areaBounds, this.location, splitIndex);
    ...
    calculateAutoPositions() {
        ...
        const areaBoundsRight = this.areaBounds.x + this.areaBounds.width;
        if (!this.inverted) {
            for (const tooltipRect of this.splitTooltipRectCollection) {
                const tooltipRightEdge = tooltipRect.x + tooltipRect.width;
                if (tooltipRightEdge > areaBoundsRight) {
                    this.hasHorizontalOverflow = true;
                }
                totalXPosition += tooltipRect.x;
            }
            const averageXPosition = totalXPosition / this.splitTooltipRectCollection.length;
            for (const tooltipRect of this.splitTooltipRectCollection) {
            ...
            const horizontalItems = this.splitTooltipRectCollection
                ...
                .sort((leftItem, rightItem) => leftItem.xPosition - rightItem.xPosition);
            for (let itemIndex = 1; itemIndex < horizontalItems.length; itemIndex++) {
                const previousItem = horizontalItems[itemIndex - 1];
                const currentItem = horizontalItems[itemIndex];
                const requiredX = previousItem.xPosition + previousItem.width + MINIMUM_GAP + OVERFLOW_BUFFER;
        ...
        const sortedTooltipItems = this.splitTooltipRectCollection
            .map((rect, originalIndex) => ({
            xPosition: rect.x,
            width: rect.width,
            yPosition: rect.y,
            ...
            .sort((firstItem, secondItem) => firstItem.yPosition - secondItem.yPosition);
        let clusterStartIndex = 0;
        let previousClusterBottomY = -Infinity;
        while (clusterStartIndex < sortedTooltipItems.length) {
            const currentCluster = [sortedTooltipItems[clusterStartIndex]];
            let nextItemIndex = clusterStartIndex + 1;
            while (nextItemIndex < sortedTooltipItems.length) {
                const previousTooltip = currentCluster[currentCluster.length - 1];
                const currentTooltip = sortedTooltipItems[nextItemIndex];
                const previousTooltipBottom = previousTooltip.yPosition + previousTooltip.height;
            ...
            if (currentCluster.length > 1) {
                let totalCenterPoints = 0;
                let clusterTotalHeight = 0;
                for (const tooltipItem of currentCluster) {
                    const centerY = tooltipItem.yPosition + (tooltipItem.height / 2);
                    totalCenterPoints += centerY;
                    clusterTotalHeight += tooltipItem.height;
                }
                const medianCenterY = totalCenterPoints / currentCluster.length;
                const totalHeightWithGaps = clusterTotalHeight + MINIMUM_GAP * (currentCluster.length - 1);
                let currentYPosition = Math.max(medianCenterY - (totalHeightWithGaps / 2), previousClusterBottomY + MINIMUM_GAP);
                for (const tooltipItem of currentCluster) {
                ...
                const singleTooltip = currentCluster[0];
                const requiredMinimumY = previousClusterBottomY + MINIMUM_GAP;
        ...
        const itemCount = sortedTooltipItems.length;
        if (itemCount === 0) {
            return;
        }
        const lastTooltip = sortedTooltipItems[itemCount - 1];
        const lastTooltipBottom = lastTooltip.yPosition + lastTooltip.height;
        const actualClipRect = this.getActualSplitClipRect();
        if (lastTooltipBottom > (actualClipRect.y + actualClipRect.height)) {
            const overflowAmount = lastTooltipBottom - actualClipRect.height + OVERFLOW_BUFFER;
            for (let currentIndex = itemCount - 1; currentIndex >= 0; currentIndex--) {
                const currentTooltipY = sortedTooltipItems[currentIndex].yPosition;
                const previousTooltipBottom = currentIndex === 0
                    ? actualClipRect.y
                    : sortedTooltipItems[currentIndex - 1].yPosition + sortedTooltipItems[currentIndex - 1].height;
                const availableSpace = currentTooltipY - previousTooltipBottom;
                if (availableSpace > overflowAmount) {
                    for (let shiftIndex = currentIndex; shiftIndex < itemCount; shiftIndex++) {
        ...
        const firstTooltip = sortedTooltipItems[0];
        if (firstTooltip.yPosition < (actualClipRect.y)) {
            const overflowAmount = actualClipRect.y - firstTooltip.yPosition + OVERFLOW_BUFFER;
            for (let currentIndex = 0; currentIndex < itemCount; currentIndex++) {
                const currentTooltipBottom = sortedTooltipItems[currentIndex].yPosition + sortedTooltipItems[currentIndex].height;
                const nextTooltipY = currentIndex === itemCount - 1
                    ? (actualClipRect.height)
                    : sortedTooltipItems[currentIndex + 1].yPosition;
                const availableSpace = nextTooltipY - currentTooltipBottom;
                if (availableSpace > overflowAmount) {
                    for (let shiftIndex = currentIndex; shiftIndex >= 0; shiftIndex--) {
                        sortedTooltipItems[shiftIndex].yPosition += overflowAmount;
                    }
                    break;
                }
    ...
    handleSplitPosition(splitIndex, groupElement, areaBounds, arrowLocation, tipLocation) {
        const pointLocationX = this.splitLocations[splitIndex].x + this.splitClipBounds[splitIndex].x;
        const pointLocationY = this.splitLocations[splitIndex].y + this.splitClipBounds[splitIndex].y;
        const isConnectorLineNeeded = pointLocationY !== (this.splitTooltipRectCollection[splitIndex].y + this.splitTooltipRectCollection[splitIndex].height / 2) && (this.seriesTypes[splitIndex] !== 'Column');
        let isLeft = false;
        if (isConnectorLineNeeded) {
            const connectorLine = this.drawConnectorLine(splitIndex, pointLocationX, pointLocationY);
        ...
        const rect = this.splitTooltipRectCollection[splitIndex];
        const tooltipOverflow = (rect.y < (0) || rect.y + rect.height > (areaBounds.height)) ||
    ...
    configureSplitTooltipElements(svgObject) {
        for (let splitIndex = 0; splitIndex < (this.split ? this.content.length : 1); splitIndex++) {
            // Group to hold text and path for each split tooltip.
            let groupElement = document.getElementById(this.element.id + '_group_' + splitIndex);
            if (!groupElement) {
    ...
    drawMarker(isBottom, isRight, size, splitIndex) {
        ...
        let shapeOption;
        let count = 0;
        const markerGroup = this.renderer.createGroup({ id: (this.split ? this.element.id + '_trackball_group_' + splitIndex : this.element.id + '_trackball_group') });
        const groupElement = getElement(this.split ? this.element.id + '_group_' + splitIndex : this.element.id + '_group');
        if (!groupElement) {
            return null;
        }
        const x = ((this.enableRTL) ? (this.split ? this.splitTooltipRectCollection[splitIndex].width
        ...
        for (let index = 0; index < this.shapes.length; index++) {
            const shape = this.shapes[this.split ? splitIndex : index];
    ...
    renderTooltipElement(areaBounds, location, splitIndex) {
        ...
        const groupElement = getElement(this.split ? this.element.id + '_group_' + splitIndex : this.element.id + '_group');
        const pathElement = getElement(this.split ? this.element.id + '_path_' + splitIndex : this.element.id + '_path');
        let rect;
        let isConnectorLineNeeded;
        let isTop = false;
        let isLeft = false;
        let isBottom = false;
        let x = 0;
        ...
        if (!isNullOrUndefined(groupElement)) {
            ...
            if (this.split) {
                const splitResult = this.handleSplitPosition(splitIndex, groupElement, areaBounds, arrowLocation, tipLocation);
    ...
    changeText(point, isBottom, isRight, splitIndex) {
        const element = document.getElementById(this.split ? this.element.id + '_text_' + splitIndex : this.element.id + '_text');
        if (isBottom) {
    ...
    renderText(isRender, splitIndex) {
        ...
        const headerContent = this.split ? '' : this.header.replace(/<b>/g, '').replace(/<\/b>/g, '').trim();
        ...
        const options = new TextOption(this.split ? this.element.id + '_text_' + splitIndex : this.element.id + '_text', this.marginX * 2, (textHeight + this.marginY * 2 + this.padding * 2 + (this.marginY === 2 ? this.controlName === 'RangeNavigator' ? 5 : 3 : 0)), anchor, '');
        ...
        const isHeader = this.split ? false : this.header !== '';
        ...
        for (let k = 0, pointsLength = this.formattedText.length; k < pointsLength; k++) {
            ...
            for (let i = 0, len = textCollection.length; i < len; i++) { // string value of unicode for LTR is \u200E
                ...
                if (this.enableRTL && lines.length > 0) {
                    const colonMatches = textCollection[i].match(/:/g);
                    const colonCount = colonMatches ? colonMatches.length : 0;
                    const shouldReverse = colonCount > 0 &&
        ...
        if (this.split) {
            const padding = 20;
            const elementWidth = this.elementSize.width + (2 * this.marginX);
            const elementHeight = this.elementSize.height + (2 * this.marginY);
            const rect = new Rect(this.splitLocations[splitIndex].x
    ...
    createTemplate(areaBounds, location, splitIndex) {
        ...
        if (this.split && !isNullOrUndefined(splitIndex)) {
            ...
            const splitParentId = this.element.id + 'parent_template' + splitIndex;
            const splitParentElem = document.getElementById(splitParentId);
            if (splitParentElem) {
        ...
        if (!argsData.cancel) {
            const elem = createElement('div', { id: this.split ? this.element.id + 'parent_template' + splitIndex : this.element.id + 'parent_template' });
            let templateElement;
            ...
            const element = this.split || this.isCanvas ? elem : this.element;
    ...
    sharedTooltipLocation(bounds, x, y) {
        const width = this.elementSize.width + (2 * this.marginX);
        const height = this.elementSize.height + (2 * this.marginY);
        const pointerTooltipRect = new Rect((this.shared || this.inverted)
            ? (x + 2 * this.padding) : (x - width / 2), (this.shared || this.inverted) ? (y - height / 2)
            : (y - (height + this.padding)), width, height);
        const tooltipRect = this.followPointer ? pointerTooltipRect : new Rect(x + 2 * this.padding, y - height - this.padding, width, height);
    ...
    tooltipLocation(bounds, symbolLocation, arrowLocation, tipLocation) {
        ...
        const clipX = this.clipBounds.x;
        const clipY = this.clipBounds.y;
        const clipWidth = this.clipBounds.width;
        const clipHeight = this.clipBounds.height;
        ...
        if (!this.inverted) {
            ...
            if (this.enableRTL && this.controlName === 'Sankey') {
                const relativeX = location.x - boundsX;
                const mirroredRelativeX = bounds.width - relativeX - (this.elementSize.width + (2 * this.marginX));
    ...
    animateTooltipDiv(tooltipDiv, rect) {
        ...
        if ((this.controlName === 'Chart' && (this.shared || this.split)) && !this.enableRTL) {
            const transformValues = (this.split ? tooltipDiv : this.element).style.transform.split(/[(),\s]+/);
            ...
            const width = this.elementSize.width + (2 * this.marginX);
            const containerWidth = (this.availableSize && this.availableSize.width) ?
                this.availableSize.width : (this.areaBounds.x + this.areaBounds.width);
            const targetRight = containerWidth - (rect.x + width);
    ...
    updateDiv(tooltipDiv, x, y) {
        if ((this.controlName === 'Chart' && ((this.shared && !this.crosshair) || this.split)) && (!this.enableRTL || this.split)) {
            ...
            const containerWidth = (this.availableSize && this.availableSize.width) ?
                this.availableSize.width : (this.areaBounds.x + this.areaBounds.width);
            const right = containerWidth - (x + width);
    ...
    fadeOut() {
        ...
        const tooltipDiv = getElement(this.element.id);
        if (tooltipElement) {
            let tooltipGroup = this.split ? tooltipElement : tooltipElement.firstChild;
