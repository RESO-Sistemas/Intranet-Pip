...
$(document).on('blur',".answerInp",function(element){
  changeValAnswer(element.target.dataset.question,$(this).val(),element.target.dataset.answer, element.target.dataset.oldanswer);
});

$(document).on("change",".changeTrue-False",function(element){
  changeexpectedValueTF(element.target.dataset.question,element.target.checked);
});

$(document).on("change",".True-FalseSV",function(element){
  changeexpectedValueTFSV(element.target.dataset.question,element.target.checked);
});
...
$(document).on("blur",".updatePrincipalInfo",async function(element){
  changePrincipalInfoQuestion(element.target.dataset.question,$(this).val(),element.target.dataset.typeinp);
});

$(document).on("blur",".updatePrincipalInfoOld", async function(element){
  changePrincipalInfoQuestionOld(element.target.dataset.question,$(this).val(),element.target.dataset.typeinp);
});
...
$(document).on("change",".selectPerQuestion",function(element){
  changeAnExpectedResponse(element.target.dataset.question,element.target.value);
});

$(document).on("change",".selectPerQuestionSV", function(element){
  let dataTypeValue = $(this).find(":selected").data("typeexpected");
  changeAnExpectedResponseSV(element.target.dataset.question,element.target.value, dataTypeValue);
});

$(document).on("blur",".changeRange", function(element){
  changeRangeValues(element.target.dataset.question, element.target.value, element.target.dataset.typenum);
});

$(document).on("blur",".changeRangeSV", function(element){
  changeRangeValuesSV(element.target.dataset.question, element.target.value, element.target.dataset.typenum);
});
  ...
  if (element.target.dataset.question !== undefined) {
    if (element.target.dataset.typequestion =="new") {
      ...
      allDvQuestionsNoS.forEach((dvQuest, index) => {
        ...
        let allchangeCompetenceSV = dvQuest.querySelectorAll(".changeCompetenceSV");
...
$(document).on("blur",".contentAnswerSVNew",function(element){
  changeValAnswerSVNew(element.target.dataset.question, element.target.value,  element.target.dataset.answersvnew);
});

$(document).on("blur",".contentAnswerSVOld", function(element){
  changeValAnswerSVOld(element.target.dataset.question, element.target.value,  element.target.dataset.answersv);
});

$(document).on("click",".changeCompetenceSV",function(element){
  console.log(element.target.dataset.question);
  $("#txQuestionPerUpdateCompetence").html(`<h6><b>Pregunta seleccionada:</b> ${Number(element.target.dataset.question) + 1}</h6>`);
  ...
  openMMinNoMaximizable('Actualización de la competencia seleccionada para la pregunta','dv_UpdateCompetence', false);
});
