    ...
    respuesta.forEach((datos) => {
      datos.Organigrama.forEach((Organigramas) => {
        $("#contenidoOrganigramas").append(`
                    <div class="col s12 l12" style="box-shadow: rgba(0, 0, 0, 0.12) 0px 1px 3px, rgba(0, 0, 0, 0.24) 0px 1px 2px; background:white; margin-top:2vh;">
                        <div class="row">
                            <div class="col s12 l12" style="text-align:center">
                                <h4>Organigrama: ${Organigramas.Titulo}</h4>
                            </div>
...
async function loadOrganigrama(Organigrama, Datos){
  ...
  if (Datos !== undefined || Datos.length > 0) {
      ...
      let diagram = new ej.diagrams.Diagram({
        ...
        tool: ej.diagrams.DiagramTools.DrawOnce | ej.diagrams.DiagramTools.ZoomPan,
...
function eventClick(args){
  if (args.name == "click" && args.actualObject !== undefined) {
    ...
    if (detformatF == "MA==") {
      toastr.info("No es posible obtener la información del elemento seleccionado.");
    } else {
      ...
      let dv = 'cDetEmp';
      if (!alertify[dv]) {
        alertify[dv] || alertify.dialog(`${dv}`,function(){
          return {
