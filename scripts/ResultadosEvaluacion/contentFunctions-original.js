...
let principalTable, table_general_detail, table_qualificationsInd, charIndv, ev_strengths, ev_weaknesses, finalTableEvaluated,
    table_final, best_final, worst_final, final_graph;
...
function printEmpleadosEvaluadosPorEv(data){
  ...
  principalTable = new ej.grids.Grid({
    ...
    toolbar: ['Search'],
    columns: [
      { field: "Empleado", headerText: "EMPLEADO EVALUADO", width: 150, textAlign: 'Center', filter: { type : 'CheckBox' }},
      { field: "IdEvaluado", headerText: "MÁS DETALLES", width: 100, textAlign: 'Center', template: "#viewIndvResultsTemplate"},
      { field: "SinCompletar", headerText: "CANTIDAD EVALUADO", width: 100, textAlign: 'Center', template: "#viewFinalReultsTemplate"},
    ],
  });
...
async function viewDetail(employee){
  ...
  if(result){
    ...
    let hd = "";
    let result = await openMMaxNoMinimizable(hd,dv);
    ...
    if (type == 1) {
      ...
      if (resultEvaluators) {
        ...
        $('#selTypeResult').select2({
            dropdownParent: $('#container-generalDetail')
        });
...
async function printCalculationParSub(){
  ...
  if (result.dataPar.length > 0) {
    result.dataPar.forEach((dataPar, index) => {
      let contentHTML = `
        <div class="col s12">
          ...
          <div class="row">
            <div class="col s12">
              <h5 style="color:#d60f0f;text-align: center"> PAR (${index + 1}) </h5>
            </div>
    ...
    $("#dv_ind_Par").append("<h4 style='text-align: center'> Sin resultados </h4>");
  }
  if (result.dataSub.length > 0) {
    result.dataSub.forEach((dataSub, index) => {
      let contentHTML = `
        <div class="col s12">
          <div class="row">
            <div class="col s12">
              <h5 style="color:#d60f0f;text-align: center"> SUBORDINADO (${index + 1}) </h5>
            </div>
            <div class="col s12 m5">
    ...
    $("#dv_ind_Subordinado").append("<h4 style='text-align: center'> Sin resultados </h4>");
  }
}
  ...
  if (data.dataJe.length > 0) {
    let contentJe = `
      <div class="col s12">
        ...
        <div class="row">
          <div class="col s12">
            <h5 style="color:#d60f0f;text-align: center"> CALIFICACIÓN DEL JEFE </h5>
          </div>
          <div class="col s12 m5">
  ...
  if (data.dataAuto.length > 0) {
    let contentAu = `
      <div class="col s12">
        ...
        <div class="row">
          <div class="col s12">
            <h5 style="color:#d60f0f;text-align: center"> CALIFICACIÓN AUTO </h5>
          </div>
          <div class="col s12 m5">
  ...
  if (data.dataPar.length > 0) {
    let contentAu = `
      <div class="col s12">
        ...
        <div class="row">
          <div class="col s12">
            <h5 style="color:#d60f0f;text-align: center"> CALIFICACIÓN DE LOS PARES </h5>
          </div>
          <div class="col s12 m5">
  ...
  if (data.dataSub.length > 0) {
    let contentAu = `
      <div class="col s12">
        ...
        <div class="row">
          <div class="col s12">
            <h5 style="color:#d60f0f;text-align: center"> CALIFICACIÓN DE LOS SUBORDINADOS </h5>
          </div>
          <div class="col s12 m5">
...
async function printResultsPerEvaluator(data){
  // const data = await getDataResultsPerEvaluator();
