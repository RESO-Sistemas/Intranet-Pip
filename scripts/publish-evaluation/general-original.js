...
function groupBranches(data){
  if (data.length > 0) {
    ...
    toastr.info("No se han encontrado sucursales dentro del sistema, inténtelo nuevamente.");
  }
}
...
function printAllTablesTempData(){
  ...
  allBranch.forEach( branch => {
    let tempBranch = allTempData.filter( data => data.IdSucursalEvaluado == branch.IdSucursal);
    if (tempBranch.length > 0) {
      let tableId = `tableB_${branch.IdSucursal}`;
      let contentHTML = `
        <div class="col s12">
          <div class="col s12 m3 offset-m9">
            <button class="btn-actionRed" onclick="dialogDeleteAllDataPerBranch('${branch.IdSucursal}')"><i class="fa-solid fa-delete-left"></i></button>
          </div>
          <div class="col s12 section scrollspy" id="card_${branch.IdSucursal}">
            <h6><b>Sucursal: </b>${branch.Sucursal}</h6>
            <div id="${tableId}"></div>
            <hr>
          </div>
        </div>
      `;
      contentSection += `
        <li><a href="#card_${branch.IdSucursal}">${branch.Sucursal}</a><hr></li>
      `;
...
function printTablePerBranch(data, tableId){
  let table = new ej.grids.Grid({
    dataSource: data,
    allowSorting: true,
    sortSettings: { columns: [{ field: 'NoEmpleadoEvaluado', direction: 'Ascending' }] },
    allowFiltering: true,
    filterSettings: { type:'Menu' },
    columns: [
      {field: "NoEmpleadoEvaluado", textHeader: "No. Evaluado", width: 100, textAlign: "Center"},
      {field: "EmpladoEvaluado", textHeader: "Evaluado", width: 100, textAlign: "Center"},
      {field: "EmpladoEvaluador", textHeader: "Evaluador", width: 100, textAlign: "Center"},
      {field: "", textHeader: "Tipo", template: "#typeEvaluatorTemplate", width: 50, textAlign: "Center"},
      {field: "", textHeader: "Activado", template: "#activeTemplate", width: 50, textAlign: "Center"},
      {field: "", textHeader: "Eliminar Evaluador", template: "#deleteTemplate", width: 50, textAlign: "Center"}
    ],
  });
...
window.deleteSF = function (e){
  ...
  let btn;
  if (e.AutoEvalua == 1) {
    btn = `<button class="btn-actionRed btn-disabled"><i class="fas fa-lock"></i></button>`;
  } else {
    btn = `<button class="btn-actionRed" onclick="deleteEvaluatorDetail('${e.idEvaluacionDetalle}')"><i class="fas fa-trash-alt"></i></button>`;
  }
  $(div).append(btn);
...
window.activeSF = function(e){
  ...
  <div class="switch">
      <label>
          <input type="checkbox" ${checked} class="updateActiveEv" data-detailev="${e.idEvaluacionDetalle}">
...
$(document).on("click",".updateActiveEv", async function(e){
  ...
  let ajaxR = await pAjaxAsync(url_m_Evaluaciones, dataSend, 1);
  if (ajaxR !== undefined) {
    let posicion = allTempData.findIndex( i => i.idEvaluacionDetalle == dataSend.detEv);
    if (posicion != "-1") {
...
async function addEmpleadoEvaluadorTempData(){
  ...
  if (ajaxR !== undefined) {
    ...
    let tempBranch = allTempData.filter( data => data.IdSucursalEvaluado == ajaxR.Data.IdSucursalEvaluado);
...
async function deleteEvaluatorDetail(detailEv){
  ...
  let ajaxR = await pAjaxAsync(url_m_Evaluaciones, dataSend, 1);
  if (ajaxR !== undefined) {
    let posicion = allTempData.findIndex( i => i.idEvaluacionDetalle == dataSend.evDetail);
    let branchSel = allTempData[posicion].IdSucursalEvaluado;
    allTempData.splice(posicion,1);
    let tempBranch = allTempData.filter( data => data.IdSucursalEvaluado == branchSel);
...
$(document).on("click","#publishEvaluation", async function(){
  let resultDial = await dialogConfirmSAlert('¿Desea publicar la evaluación con el detalle de los evaluadores registrados y aceptados?');
  if (resultDial) {
...
async function getInitialConfigEvaluation() {
  ...
  if (ajaxR != undefined) {
    ...
    if (ajaxR.Data.TipoSeleccionaSucursal == null && ajaxR.Data.TipoOpcionConfiguracion == null) {
