...
function printDetalleEv(dataR) {
  ...
  dataR.forEach((allC, index) => {
    ...
    if (allC.idTipoPregunta == 1) {
      ...
      contentConfig = `
        <div class="col" id="contentValues${contador}" data-typeq="1" data-answeres="${allC.idRespuestaEvaluaciones}">
          <div class="switch d-flex flex-column align-items-center">
            <label class="form-label mb-1">Falso / Verdadero</label>
            <div class="form-check form-switch">
              <input type="checkbox" class="form-check-input  trueOrFalse" ${value}>
            </div>
          </div>
          ...
          <div class="row">
            <div class="col">
              <div class="input-field m-t-20">
                ...
                <textarea  class="form-control" placeholder="Ingrese algún comentario (opcional)">${allC.Comentarios}</textarea>
              </div>
            </div>
          </div>
        </div>`;
  ...
  <div class="col d-flex justify-content-center mb-3">
    <div class="form-check d-flex flex-column align-items-center">
      <input 
        ...
        type="radio" 
        name="ans${index}" 
        id="ans${answer.idPreguntasPosiblesRespuestas}" 
        data-answer="${answer.idPreguntasPosiblesRespuestas}" 
        ${value}
      >
      <label 
        class="form-check-label text-center" 
        for="ans${answer.idPreguntasPosiblesRespuestas}">
        ${answer.DescripcionRespuesta}
      </label>
    </div>
  </div>
`;
      ...
      contentConfig = `
  <div class="col" id="contentValues${contador}" data-typeq="${allC.idTipoPregunta}" data-answeres="${allC.idRespuestaEvaluaciones}">
    ...
    <div class="row">
      ...
      contentConfig = `
        <div class="col s12 m6" id="contentValues${contador}" data-typeq="3" data-answeres="${allC.idRespuestaEvaluaciones}">
          <label class="form-label pb-2">En las preguntas de tipo rango, es necesario ingresar un valor el cual se encuentre entre el rango inicial y el rango final.</label >
          <div class="row">
            <div class="col s6 m3"  style="text-align:center;">
              <label class="form-label">Rango Inicial:</label>
              <input class="form-control form-control-solid-bordered m-b-sm" type="number" data-typen="initial" data-range="${allC.Config.RangoInicial}" value="${allC.Config.RangoInicial}" readonly  style="text-align:center;"/>
            </div>
            <div class="col s6 m3"  style="text-align:center;">
              <label class="form-label">Rango Final:</label>
              <input class="form-control form-control-solid-bordered m-b-sm" type="number" data-typen="end" data-ramge="${allC.Config.RangoFinal}" value="${allC.Config.RangoFinal}" readonly  style="text-align:center;"/>
            </div>
            <div class="col s12 m6"  style="text-align:center;">
              <label class="form-label">Ingrese el valor:</label>
              <input type="number" class="form-control text-center valueRange" min="${allC.Config.RangoInicial}" value="${value}">
            </div>
          </div>
                    <div class="row">
            <div class="col">
      ...
      <div class="card  p-4 mb-4">
        <div class="card-body">
          ...
          <h6 class="fw-bold mb-2" style="font-size: 1.1rem;">
            <span class="text-muted">Competencia:</span> ${allC.Competencia}
          </h6>
          <h6 class="fw-bold" style="font-size: 1.1rem;">
            <span class="text-muted">Pregunta:</span> ${allC.Descripcion}
          </h6>
        </div>
        ...
        const messageContent = `
            <div class="alert-content">
             <span class="alert-title">Alerta!</span>
              <span class="alert-text">Ha ocurrido un error al evaluar la competencia.</span>
            </div>`;
    ...
    const messageContent = `
        <div class="alert-content">
             <span class="alert-title">Información!</span>
              <span class="alert-text">Ingrese una calificación válida para poder continuar.</span>
        </div>`;
