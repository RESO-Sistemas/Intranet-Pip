...
<body>
							...
							<div class="col-12 col-lg-8">
								...
								<div class="post-compose-box" id="postComposeBox">
									...
									<div class="post-compose-top" id="postComposeTriggerRow">
										<button class="post-compose-trigger" id="postComposeTriggerBtn"
										...
										<div class="post-compose-form">
											<form id="formFeed">
												<div class="mb-3">
													...
													<textarea id="mnf_title" name="mnf_title" class="form-control"
														rows="2" required
														placeholder="Escribe el título de tu publicación..."></textarea>
													<p for="mnf_title" data-msg="El título es obligatorio."
														class="text-danger small mb-0"></p>
												</div>
												<div class="mb-3">
													<label for="mnf_desc" class="form-label">* Descripción</label>
													<textarea id="mnf_desc" name="mnf_desc" class="form-control"
														rows="3" required
														placeholder="¿Qué quieres comunicar?"></textarea>
													<p for="mnf_desc" data-msg="La descripción es obligatoria."
														class="text-danger small mb-0">
													</p>
												</div>
												<div class="mb-3">
													<label for="mnf_url" class="form-label">
														Hipervínculo <span class="text-muted fw-normal"
															style="font-size:.75rem;">(opcional)</span>
													</label>
												...
												<div class="mb-2">
													<label class="form-label">
														Imágenes <span class="text-muted fw-normal"
															style="font-size:.75rem;">(opcional)</span>
													</label>
													<div id="fileUpload"
														class="file-container border rounded p-3 bg-light"></div>
												</div>
											</form>
											<div class="post-compose-form-footer">
												<button class="btn-cancel-compose" type="button"
											...
											<div id="composePublishStatus" class="small text-muted mt-2 d-none">
												<i class="fa fa-spinner fa-spin me-1"></i> Publicando tu post, espera un
												momento...
											</div>
										</div>
									</div>
			  ...
			  ═══════════════════════════════════════ -->
							<div class="col-12 col-lg-4">
								<div class="reddit-sidebar">
									...
									<div class="card mt-3" id="card-pending-evals">
										<div
											class="card-header text-dark d-flex justify-content-between align-items-center">
											<span><i class="fas fa-clipboard-list"></i> Evaluaciones pendientes</span>
											<span class="badge bg-danger rounded-pill" id="badge-pending-evals"
												style="display:none"></span>
										</div>
										<div class="card-body p-2">
											<div id="lista-pending-evals">
												<p class="text-muted small text-center mb-0">Cargando...</p>
											</div>
										</div>
									</div>
			...
			$.ajax({
				...
				success: function (response) {
					...
					if (!eventos || eventos.length === 0) {
						el.innerHTML =
							'<p class="text-muted small text-center py-2">No hay eventos próximos</p>';
						return;
					}
					var meses = ['Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Ago', 'Sep', 'Oct', 'Nov',
						'Dic'
					];
					var html = '';
	...
	<script>
		async function loadPendingEvalsWidget() {
			try {
				...
				const badge = document.getElementById('badge-pending-evals');
				const lista = document.getElementById('lista-pending-evals');
				if (!lista) return;

				if (items.length === 0) {
					if (badge) badge.style.display = 'none';
					lista.innerHTML = '<p class="text-muted small text-center mb-0 py-1">Sin evaluaciones pendientes</p>';
					return;
				}
				...
				lista.innerHTML = items.map(function (e) {
					return '<a href="pending-evaluations.php" class="d-block text-decoration-none py-2 px-1 border-bottom">' +
						...
						'</small>' +
						'<small class="text-muted">Vence: ' + e.FechaFin + '</small>' +
						'</a>';
				}).join('');
			} catch (err) {
				const lista = document.getElementById('lista-pending-evals');
				if (lista) lista.innerHTML = '<p class="text-muted small text-center mb-0 py-1">Error al cargar</p>';
			}
		}

		loadPendingEvalsWidget();
		if (typeof NotificationManager !== 'undefined') {
			NotificationManager.onRefresh(loadPendingEvalsWidget);
		}
	</script>
